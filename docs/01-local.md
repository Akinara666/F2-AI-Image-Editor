...
| `SD_ENABLE_VAE_SLICING` | `true` | VAE slicing (режет batch) — дёшево, экономит VRAM на декоде |
| `SD_ENABLE_VAE_TILING` | `auto` | VAE tiling: режет финальный декод на тайлы (экономит VRAM, но добавляет проходы → последний шаг дольше). Пусто = авто (следует за `SD_ENABLE_CPU_OFFLOAD`); `true` / `false` — принудительно |
