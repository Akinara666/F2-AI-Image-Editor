...
| Переменная | По умолчанию | Назначение |
|---|---|---|
| `SERVE_FRONTEND` | `false` | `true` — backend отдаёт собранный фронт на том же origin (один URL = сайт). На vast включается `run-vast.sh --with-frontend`; в Docker не нужно (фронт отдаёт nginx) |
