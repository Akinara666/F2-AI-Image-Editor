...
## 📂 Управление Моделями
Проект поддерживает как загрузку с HuggingFace, так и локальные файлы.

### Куда класть файлы?
*   `backend/models/` — для чекпоинтов (`.safetensors`, `.ckpt`).
*   `backend/loras/` — для LoRA адаптеров.

### Как использовать?
1.  **HuggingFace**: Просто укажите ID (например, `runwayml/stable-diffusion-v1-5`).
2.  **Локальные файлы**: Укажите **абсолютный путь** к файлу.
    *   Пример: `/home/user/projects/psd2/backend/models/my_model.safetensors`.
